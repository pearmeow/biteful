...
void FoodItems::create(const HttpRequestPtr& req, std::function<void(const HttpResponsePtr&)>&& callback) {
    ...
    try {
        auto result = dbClient->execSqlSync(
            "INSERT INTO food_items (menu_id, dish_name, menu_section, dish_desc, price, health_points) VALUES "
            "($1, $2, $3, $4, "
            "$5, $6) RETURNING id",
        ...
        std::string prompt =
            ...
            "Score each dish as an integer from -10 to 10 where +10 = extremely healthy and -10 = extremely "
            "unhealthy.\n\n"
            ...
            "High-fat sauce (peanut sauce, coconut curry, tahini-heavy, cream sauce, aioli, ranch, mayo, "
            "alfredo)\n"
            "Carbs: Whole (vegetables, fruits, legumes, whole grains) | Refined (white bread, white rice, pasta, "
            "chips, fries)\n"
            "Dessert: Any item whose primary identity is sweet or dessert-based (cake, ice cream, gelato, "
            "tiramisu, mousse, churros, etc.)\n"
            ...
            "2. Lean protein (+4) and healthy prep (+2) may both apply only if prep is stated or implied by dish "
            "name.\n"
            "3. Fried or dessert items: cap score at 0 unless two or more strong positive signals (+2 or higher) "
            "are present.\n"
            ...
            "7. Verify before returning: desserts must score negative, lean protein with healthy prep must score "
            "moderately to strongly positive, fried items must score negative.\n\n"
            ...
            "-7 to -10: Reserved for extreme cases - deep-fried desserts, pure sugar confections, items with "
            "near-zero nutritional value\n\n"
        ...
        drogon::app().getLoop()->runAfter(staggerDelay, [client, claudeReq, dbClient, newId]() {
            client->sendRequest(claudeReq, [dbClient, newId](drogon::ReqResult res,
                ...
                if ((*respJson).isMember("error")) {
                    LOG_ERROR << "Claude API error for id " << newId << ": "
                              << (*respJson)["error"]["message"].asString();
                    return;
                }
                ...
                dbClient->execSqlAsync(
                    "UPDATE food_items SET health_points = $1 WHERE id = $2", [](const drogon::orm::Result&) {},
